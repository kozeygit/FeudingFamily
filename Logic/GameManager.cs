...
public class GameManager : IGameManager
{
    ...
    public JoinGameResult GameKeyValidator(string? gameKey)
    {
        ...
        if (string.IsNullOrWhiteSpace(gameKey)) return new JoinGameResult { ErrorCode = JoinErrorCode.KeyEmpty };

        if (gameKey.Length != 4) return new JoinGameResult { ErrorCode = JoinErrorCode.KeyWrongLength };
    ...
    public async Task OnGameBuzz(object? sender, (string gameKey, Team team) args)
    {
        ...
        if (gameResult.Success is false) return;
    ...
    public JoinGameResult GetGame(string gameKey)
    {
        ...
        if (!games.TryGetValue(gameKey, out var game))
            return new JoinGameResult { ErrorCode = JoinErrorCode.GameNotFound };

        return new JoinGameResult { Game = game };
    }

    public async Task<JoinGameResult> JoinGame(string gameKey, string connectionId, ConnectionType connectionType,
        Guid? teamID)
    {
        if (!games.TryGetValue(gameKey, out var game))
            return new JoinGameResult { ErrorCode = JoinErrorCode.GameNotFound };
        ...
        if (connectionType == ConnectionType.Buzzer && teamID is not null)
        {
            ...
            if (team is null) return new JoinGameResult { ErrorCode = JoinErrorCode.TeamNameInvalid };
        ...
        if (connectionType == ConnectionType.EspBuzzer)
        {
            ...
            team.AddMember(connection);
            await _hubContext.Clients.All.SendAsync("receiveTeams", game.Teams.Select(t => t.MapToDto()));
        }
    ...
    public void LeaveGame(string gameKey, string connectionId)
    {
        ...
        if (games.TryGetValue(gameKey, out var game) is false) return;

        if (gameRooms.TryGetValue(gameKey, out var gameRoom) is false) return;
    ...
    public (Game? game, GameConnection? connection) ValidateGameConnection(string gameKey, string connectionId)
    {
        ...
        if (joinGameResult.Success is false) return default;

        var connection = GetConnection(gameKey, connectionId);

        if (HasConnection(gameKey, connection) is false) return default;
    ...
    public static string GetErrorMessage(Enum errorCode)
    {
        ...
        return custAttr?.SingleOrDefault() is not DescriptionAttribute attribute
            ? errorCode.ToString()
...
public interface IGameManager
{
    ...
    Task<JoinGameResult> JoinGame(string gameKey, string connectionId, ConnectionType connectionType, Guid? teamID);
